    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for iter_id, train_data in enumerate(train_loader):
            ...
            if opt['network_G']['which_model_G'] == 'LIIF':
                ...
                fake_H = fake_H[:, :, :, 0:H, 0:W].reshape(b*n,3,H,W)
            import torchvision
            diff = abs(fake_H-real_H).sum(1,True).repeat(1,3,1,1)
            '''path = "Our12/"
            try:
                import os
                os.mkdir("test/"+path)
                ...
                pass
            for i in range(n-1):
                torchvision.utils.save_image(real_H[i], "test/"+path+"GT"+str(iter_id*8+i)+".png")
                torchvision.utils.save_image(fake_H[i], "test/"+path+str(iter_id*8+i)+".png")
                torchvision.utils.save_image(diff[i], "test/"+path+"diff"+str(iter_id*8+i)+".png")'''
            ...
            print(psnr_all, np.mean(psnr_all[:-1]))
            ...
            '''for idx in range(n-1):
                ssim = util.calculate_ssim(real_H[idx:idx+1].permute(1,2,0).cpu().detach().numpy(), fake_H[idx:idx+1].permute(1,2,0).cpu().detach().numpy())
                ssims.append(ssim)'''
