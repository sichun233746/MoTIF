...
kernel_Softsplat_updateOutput = '''
    ...
    const int n,
        ...
        if ((intNorthwestX >= 0) & (intNorthwestX < SIZE_3(output)) & (intNorthwestY >= 0) & (intNorthwestY < SIZE_2(output))) {
            atomicMaxFloat(&output[OFFSET_4(output, intN, intC, intNorthwestY, intNorthwestX)], VALUE_4(input, intN, intC, intY, intX) * fltNorthwest);
        }
        if ((intNortheastX >= 0) & (intNortheastX < SIZE_3(output)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(output))) {
            atomicMaxFloat(&output[OFFSET_4(output, intN, intC, intNortheastY, intNortheastX)], VALUE_4(input, intN, intC, intY, intX) * fltNortheast);
        }
        if ((intSouthwestX >= 0) & (intSouthwestX < SIZE_3(output)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(output))) {
            atomicMaxFloat(&output[OFFSET_4(output, intN, intC, intSouthwestY, intSouthwestX)], VALUE_4(input, intN, intC, intY, intX) * fltSouthwest);
        }
        if ((intSoutheastX >= 0) & (intSoutheastX < SIZE_3(output)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(output))) {
            atomicMaxFloat(&output[OFFSET_4(output, intN, intC, intSoutheastY, intSoutheastX)], VALUE_4(input, intN, intC, intY, intX) * fltSoutheast);
        }
    } }
'''
...
kernel_Softsplat_updateGradInput = '''
    extern "C" __global__ void kernel_Softsplat_updateGradInput(
        ...
        if ( (intNorthwestX >= 0) & (intNorthwestX < SIZE_3(gradOutput)) & (intNorthwestY >= 0) & (intNorthwestY < SIZE_2(gradOutput)) &
             (VALUE_4(output, intN, intC, intNorthwestY, intNorthwestX) == VALUE_4(input, intN, intC, intY, intX) * fltNorthwest) ) {
        ...
        if ( (intNortheastX >= 0) & (intNortheastX < SIZE_3(gradOutput)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(gradOutput)) &
             (VALUE_4(output, intN, intC, intNortheastY, intNortheastX) == VALUE_4(input, intN, intC, intY, intX) * fltNortheast) ) {
        ...
        if ( (intSouthwestX >= 0) & (intSouthwestX < SIZE_3(gradOutput)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(gradOutput)) &
             (VALUE_4(output, intN, intC, intSouthwestY, intSouthwestX) == VALUE_4(input, intN, intC, intY, intX) * fltSouthwest) ) {
        ...
        if (  (intSoutheastX >= 0) & (intSoutheastX < SIZE_3(gradOutput)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(gradOutput)) &
             (VALUE_4(output, intN, intC, intSoutheastY, intSoutheastX) == VALUE_4(input, intN, intC, intY, intX) * fltSoutheast)) {
...
class _FunctionSoftsplat(torch.autograd.Function):
    ...
    def forward(self, input, flow):
        ...
        output = input.new_ones([ intSamples, intInputDepth, intInputHeight, intInputWidth ]) 
        ...
        self.save_for_backward(input, flow, output)
    ...
    def backward(self, gradOutput):
        input, flow, output = self.saved_tensors
        ...
        if input.is_cuda == True:
            if gradInput is not None:
                ...
                cupy_launch('kernel_Softsplat_updateGradInput', cupy_kernel('kernel_Softsplat_updateGradInput', {
                    ...
                    grid=tuple([ int((n + 512 - 1) / 512), 1, 1 ]),
                    block=tuple([ 512, 1, 1 ]),
                    args=[ cupy.int32(n), input.data_ptr(), flow.data_ptr(), gradOutput.data_ptr(), gradInput.data_ptr(), None , output.data_ptr()],
                    stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
                )
            # end
            if gradFlow is not None:
                ...
                cupy_launch('kernel_Softsplat_updateGradFlow', cupy_kernel('kernel_Softsplat_updateGradFlow', {
                    ...
                    grid=tuple([ int((n + 512 - 1) / 512), 1, 1 ]),
                    block=tuple([ 512, 1, 1 ]),
                    args=[ cupy.int32(n), input.data_ptr(), flow.data_ptr(), gradOutput.data_ptr(), None, gradFlow.data_ptr(), output.data_ptr() ],
                    stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
                )
            # end
        elif input.is_cuda == False:
...
def FunctionSoftsplat(tenInput, tenFlow):
    tenMax = _FunctionSoftsplat.apply(tenInput, tenFlow)

    return tenMax
# end

class Softsplat_Max(nn.Module):
    def __init__(self):
        super(Softsplat_Max, self).__init__()
    # end
