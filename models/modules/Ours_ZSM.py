...
class LunaTokis(nn.Module):
    def __init__(self, setting = 5):
        ...
        self.warp_to_many = False if setting < 6 else True
        ...
        if not self.siren:
            ...
            self.synth_net = Siren(in_features=65, out_features=3, hidden_features=[64, 64, 64, 256],
    ...
    def forward(self, x, input_target_frames, target_t, scale = None, rank = 0, train_idx = 0, use_GT = True, iter = 12, flows = None):
        ...
        hr_coord = hr_coord.to(residual.device)
        eps_shift = 1e-6
        rx = 2 / H / 2
        ry = 2 / W / 2        
        ...
        for vx in vx_lst:
            for vy in vy_lst:    
                ...
                warped = F.grid_sample(
                    residual, coord_.flip(-1).unsqueeze(1).repeat(B,1,1,1),
                    mode='nearest', align_corners=False)[:, :, 0, :]
                q_residual = warped.reshape(B,-1,HH*WW).permute(0,2,1)
        ...
        return torch.clamp((output_all), 0, 1), 0 , 0
