...
class LunaTokis(nn.Module):
    def __init__(self, setting = 5):
        ...
        if not self.siren:
            ...
            self.synth_net = Siren(in_features=192+5*self.groups+64*self.res_liff+self.decoder_Z+self.warp_to_many*133-5+2, out_features=3, hidden_features=[64, 64, 64, 256],
    ...
    def forward(self, x, input_target_frames, target_t, scale = None, rank = 0, train_idx = 0, use_GT = True, iter = 12, flows = None):
        ...
        with torch.no_grad():
            ...
            if False:
                ...
                flow = self.flow_predictor (torch.cat([fr0, fr1, fr0, fr1], dim=0)*255.,
                                            torch.cat([fr0, fr0, fr1, fr1], dim=0)*255.,                                            
                                            iters = iter
                                            )[-1]
                fr0, fr1 = x[:, :, 0], x[:, :, 1]
                ...
                # Z importance metric
                warped, _ = self.bwarp(
                                        torch.cat([fr0, fr0, fr1, fr1], dim=0)
                                    , flow
                                    )
                psi_photo = torch.nn.functional.l1_loss(
                                input=torch.cat([fr0, fr1, fr0, fr1], dim=0)
            ...
            if self.training:
                ...
                flow_GT = self.flow_predictor (
                                            t_frs.unsqueeze(0).repeat(2,1,1,1,1,1).reshape(2*B*N,3,128,128)*255.,
                                            torch.cat([t_fr0, t_fr1], dim=0).repeat(1,N,1,1).reshape(2*B*N,3,128,128)*255.,
                                            iters = iter
                                            )[-1]
        ...
        if use_GT:
            output, _ = self.bwarp(feat, flow_GT)
        else:
            output, _ = self.bwarp(feat, flow)
        ...
        if not self.warp_to_many:
            ...
            output_all = torch.cat((
                    ...
                    q_residual.repeat(1,N,1,1).reshape(B*N,-1,HH,WW),
                    target_t.reshape(B*N,1,1,1).repeat(1,1,HH,WW), 
                    z.reshape(2,B*N, -1, HH,WW).permute(1,0,2,3,4).reshape(B*N,-1,HH,WW)
                    ), 1).reshape(B*N, -1, HH,WW)
        if not self.siren:
