...
class LunaTokis(nn.Module):
    ...
    def forward(self, x, input_target_frames, target_t, scale = None, rank = 0, train_idx = 0, use_GT = True, iter = 12, flows = None):
        ...
        with torch.no_grad():
            ...
            if flows is not None:
                ...
                x_norm = interpolate(x.reshape(B,-1,H,W),
                                scale_factor = scale, mode='bilinear', align_corners=False).reshape(B,-1,2,HH,WW)
                fr0, fr1 = x_norm[:, :, 0], x_norm[:, :, 1]
                flow = self.flow_predictor (torch.cat([fr0, fr0, fr1, fr1], dim=0)*255.,
                                            torch.cat([fr0, fr1, fr0, fr1], dim=0)*255.,
                                            iters = iter
                                            )[-1]
                fr0, fr1 = x[:, :, 0], x[:, :, 1]
                flow = interpolate(flow, size=(H, W), mode='bilinear', align_corners=False) * (1 / scale )                        
            ...
            # Z importance metric
            warped, _ = self.bwarp(
                                    torch.cat([fr0, fr1, fr0, fr1], dim=0)
                                , flow
                                )
            psi_photo = torch.nn.functional.l1_loss(
                            input=torch.cat([fr0, fr0, fr1, fr1], dim=0)
                                                    , target=warped, reduction='none').mean(1)
            flow = flow.reshape(4, B, 2 , H, W)
            warped, _ = self.bwarp(
                                -torch.cat([flow[0], flow[2], 
                                            flow[1], flow[3], 
                                            ], dim=0
                                            )
                                , flow.reshape(4*B, 2 , H, W))
            psi_flow = torch.nn.functional.l1_loss(input=(flow.reshape(4*B, 2 , H, W))
                                                            , target=warped, reduction='none').mean(1)
            ...
            if (flows is not None) and use_GT:
                ...
                flow_GT = self.flow_predictor (torch.cat([t_fr0, t_fr1], dim=0).repeat(1,N,1,1).reshape(2*B*N,3,HH,WW)*255.,
                                            t_frs.unsqueeze(0).repeat(2,1,1,1,1,1).reshape(2*B*N,3,HH,WW)*255.,
                                            iters = iter
                                            )[-1]
        ...
        flow, z = flow[:,:-1] *20. * scale , (nn.ReLU()(flow[:,-1].unsqueeze(1)) * self.alpha)
        ...
        return torch.clamp((output_all), 0, 1), flow/20.0/scale , (flow_GT/20.0)/scale
