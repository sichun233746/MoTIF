...
class Net(nn.Module):
    def __init__(self):
        ...
        self.flow = UNet(6, 4).to(self.device)
        self.interp = UNet(20, 5).to(self.device)
        self.back_warp = None
        states = torch.load("/home/abcd233746pc/Super-SloMo/SuperSloMo.ckpt", map_location='cpu')
        self.interp.load_state_dict(states['state_dictAT'])
        self.flow.load_state_dict(states['state_dictFC'])
        self.h0, self.w0 = 180*2, 312*2
        self.w, self.h = (self.w0 // 32) * 32, (self.h0 // 32) * 32
        self.setup_back_warp(self.w, self.h)
        self.norm = nn.Parameter(torch.FloatTensor([0.429, 0.431, 0.397]), requires_grad=False).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(self.device)
    def setup_back_warp(self, w, h):
        with torch.set_grad_enabled(False):
            self.back_warp = backWarp(w, h, self.device).to(self.device)

    def forward(self, frames, factor):
        frame0 = frames[:,0] - self.norm
        frame1 = frames[:,1] - self.norm
        ...
        frame_buffer = [
            interpolate(i0+self.norm,
                        size=(self.h0,self.w0), mode='bilinear', align_corners=False)
                        ]
        for i in range(1, factor):
            ...
            frame_buffer.append(
            interpolate(ft_p+self.norm,
                        size=(self.h0,self.w0), mode='bilinear', align_corners=False)                
            )
        frame_buffer.append(
        interpolate(i1+self.norm,
                    size=(self.h0,self.w0), mode='bilinear', align_corners=False)                
        )
        return torch.stack(frame_buffer, 0)