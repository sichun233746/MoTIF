...
class LunaTokis(nn.Module):
    def __init__(self, setting = 5):
        ...
        self.decoder_Z = False if setting < 5 else True
        ...
        ckpt = torch.load("/home/abcd233746pc/LunaTokis.pth")
        ...
        if not self.trans:
            self.flow_process = nn.Sequential(
                nn.Conv2d(16 + self.input_Z*12, channel, 3, 1, 1, bias=True, groups = 4),
    ...
    def forward(self, x, input_target_frames, target_t, scale = None, rank = 0, train_idx = 0, use_GT = True, iter = 12, flows = None):
        #print(x.shape, input_target_frames.shape, len(target_t), scale, flows[0].shape, flows[1].shape)
        #print(target_t)
        ...
        with torch.no_grad():
            ...
            if flows is not None:
                flow, flow_GT = flows
                flow = flow.permute(1,0,2,3,4).reshape(16*B,2,H,W).float()
            else:
                x_norm = interpolate(x.reshape(B,-1,H,W),
                                scale_factor = scale, mode='bilinear', align_corners=False).reshape(B,-1,4,HH,WW)
                fr0, fr1, fr2, fr3 = x_norm[:, :, 0], x_norm[:, :, 1], x_norm[:, :, 2], x_norm[:, :, 3]
                flow = self.flow_predictor (torch.cat([fr0, fr0, fr0, fr0, fr1, fr1, fr1, fr1, fr2, fr2, fr2, fr2, fr3, fr3, fr3, fr3], dim=0)*255.,
                                            torch.cat([fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3], dim=0)*255.,
                                            iters = iter
                                            )[-1]
                fr0, fr1, fr2, fr3 = x[:, :, 0], x[:, :, 1], x[:, :, 2], x[:, :, 3]
                flow = interpolate(flow, size=(H, W), mode='bilinear', align_corners=False) * (H / HH)                       
            
            flow = flow.reshape(16,B,2, H, W)
            flow[0 ] *= 0.
            flow[5 ] *= 0.
            flow[10] *= 0.
            flow[15] *= 0.
            ...
            # Z importance metric
            warped, _ = self.bwarp(
                                    torch.cat([fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3, fr0, fr1, fr2, fr3], dim=0)
                                , flow
                                )
            psi_photo = torch.nn.functional.l1_loss(
                            input=torch.cat(  [fr0, fr0, fr0, fr0, fr1, fr1, fr1, fr1, fr2, fr2, fr2, fr2, fr3, fr3, fr3, fr3], dim=0)
                                                    , target=warped, reduction='none').mean(1)
            flow = flow.reshape(16, B, 2 , H, W)
            warped, _ = self.bwarp(
                                -torch.cat([flow[0], flow[4], flow[8 ], flow[12],
                                            flow[1], flow[5], flow[9 ], flow[13],
                                            flow[2], flow[6], flow[10], flow[14],
                                            flow[3], flow[7], flow[11], flow[15],
                                            ], dim=0
                                            )
                                , flow.reshape(16*B, 2 , H, W))
            psi_flow = torch.nn.functional.l1_loss(input=(flow.reshape(16*B, 2 , H, W))
                                                            , target=warped, reduction='none').mean(1)
            f = (flow).reshape(16*B, -1, H, W)
            ...
            #else:
            if self.training:
                flow_GT = flow_GT.reshape(B,N,4,2,HH,WW).permute(2,0,1,3,4,5).reshape(4*B*N,2,HH,WW)
            else:
                flow_GT = 0
                ...
                t_fr0, t_fr1, t_fr2, t_fr3 = input_target_frames[:, 1],input_target_frames[:, 3],input_target_frames[:, 5],input_target_frames[:, 7]
                t_frs = input_target_frames[:, 1:-1]
                flow_GT = self.flow_predictor (torch.cat([t_fr0, t_fr1,t_fr2, t_fr3], dim=0).repeat(1,N,1,1).reshape(4*B*N,3,128,128)*255.,
                                            t_frs.unsqueeze(0).repeat(4,1,1,1,1,1).reshape(4*B*N,3,128,128)*255.,
                                            iters = iter
                                            )[-1]
            ...
            feat = self.encoder(torch.stack([fr0, fr1, fr2, fr3], 1), None)
            #  feat B 7 C H W
            residual = feat[:,int(target_t.item()*6)].reshape(B,-1,H,W)
            #residual = feat.reshape(B,-1,H,W)
            feat = torch.cat((feat[:,0],feat[:,2],feat[:,4],feat[:,6]), 0)
            # residual B NC H W
            pyramid.append(feat)
            flow = flow.reshape(16*B, 2 , H, W)
            ref_start_durations=[
        ...
        if not self.trans:
            flow_feat = torch.cat(
                (
                (flow/20.).reshape(4, 4, B,-1,H,W).permute(0,2,1,3,4,5).reshape(4*B,4,-1,H,W),
                psies.reshape(4, 4, B,-1,H,W).permute(0,2,1,3,4,5).reshape(4*B,4,-1,H,W) ,
                ref_start_durations.reshape(4,8,1,1).unsqueeze(1).repeat(1,B,1,H,W).reshape(4*B,4,2,H,W)/6.0 ,
                )
                , dim = 2).reshape(4*B,-1,H,W)
        ...
        for vx in vx_lst:
            for vy in vy_lst:    
                ...
                c1, c3, c4, c5 = 4*B*warpped_feat.shape[1], 4*B*flow_feat.shape[1], feat_coord.shape[1], residual.shape[1]*B
                ...
                q_feat = q_feat.reshape(4*B,-1,HH*WW).permute(0,2,1)
                q_flow_feat = q_flow_feat.reshape(4*B,-1,HH*WW).permute(0,2,1)
                ...
                q_flow_feat, q_feat  = torch.cat([q_flow_feat.repeat(1,N,1).reshape(4*B*N,qs,-1), 
                                                target_t.reshape(B*N,1,1).repeat(4,qs,1),
                                                rel_coord.repeat(4*B*N,1,1)
                                                ], dim=-1), \
                                    torch.cat([q_feat.reshape(4*B,qs,-1), 
                                                rel_coord.repeat(4*B,1,1),
                                                ], dim=-1)                              
        ...
        feat = q_feat.reshape(4*B,HH,WW,-1).permute(0,3,1,2)
        feat_low = q_feat_low.reshape(4*B,HH,WW,-1).permute(0,3,1,2)
        q_residual = q_residual.reshape(B,HH,WW,-1).permute(0,3,1,2).reshape(B*N,-1,HH,WW)
        if self.res_liff:
            q_residual_low = q_residual_low.reshape(B*N,HH,WW,-1).permute(0,3,1,2)
            q_residual = torch.cat([q_residual, q_residual_low], 1)
        flow = q_flow_feat.reshape(4*B*N,HH,WW,-1).permute(0,3,1,2).reshape(4*B*N*self.groups, -1, HH, WW)
        #if use_GT:
        #if False:
        if self.training:
            flow_GT = flow_GT.repeat(1,self.groups,1,1).reshape(4*B*N*self.groups, -1, HH, WW)
        feat = torch.cat([feat.repeat(1,N,1,1).reshape(4*B*N*self.groups, -1, HH, WW), 
                          flow[:,:-1].detach(),
                          feat_low.repeat(1,N,1,1).reshape(4*B*N*self.groups, -1, HH, WW)
                          ], 1)
        ...
        if not self.warp_to_many:
            output = output.reshape(4,B*N*self.groups,-1,HH,WW).sum(0)
            warped_z = warped_z.reshape(4,B*N*self.groups,-1,HH,WW).sum(0)
            warped_z[warped_z==0] = 1.0
            output /= warped_z
            z_max = z_max.reshape(4,B*N*self.groups,-1,HH,WW).max(0)[0]
            count = count.reshape(4,B*N*self.groups,-1,HH,WW).sum(0)
        else:
        ...
        if not self.warp_to_many:
            output_all = torch.cat((
                    ...
                    q_residual.reshape(B*N,-1,HH,WW),
