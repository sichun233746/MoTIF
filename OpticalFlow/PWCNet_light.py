...
class PWCNet(torch.nn.Module):
    def __init__(self):
        ...
        self.in_normalize = torch.nn.InstanceNorm2d(3, affine=True)#transforms.Normalize([0.429, 0.431, 0.397], [1.0, 1.0, 1.0])
        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                intCurrent = \
                [None, None, 81 + 32 + 2, 81 + 64 + 2, 81 + 96 + 2, 81 + 128 + 2, 81, None][
                    intLevel + 0]
                ...
                self.moduleThr = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=128, out_channels=96, kernel_size=3, stride=1,
                                    padding=1),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
                )

                self.moduleFou = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=96, out_channels=64, kernel_size=3, stride=1,
                                    padding=1),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
                )
            ...
            def Backward(self, tensorInput, tensorFlow, Backward_tensorGrid, Backward_tensorPartial):
                if str(tensorFlow.size()) not in Backward_tensorGrid:
                    ...
                    Backward_tensorGrid[str(tensorFlow.size())] = torch.cat([tensorHorizontal, tensorVertical],
                                                                            1).cuda(device = tensorInput.device, non_blocking=True)
                # end
            ...
            def forward(self, tensorFirst, tensorSecond, objectPrevious, Backward_tensorGrid, Backward_tensorPartial):
                ...
                tensorFeat = self.moduleOne(tensorFeat)
                tensorFeat = self.moduleTwo(tensorFeat)
                tensorFeat = self.moduleThr(tensorFeat)
                tensorFeat = self.moduleFou(tensorFeat)
                tensorFeat = self.moduleFiv(tensorFeat)
        ...
        class Refiner(torch.nn.Module):
            def __init__(self):
                ...
                self.moduleMain = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=81 + 32 + 2, out_channels=128,
    ...
    def forward_pre(self, tensorFirst, tensorSecond):
        ...
        return objectEstimate['tensorFlow']
# end
# end
