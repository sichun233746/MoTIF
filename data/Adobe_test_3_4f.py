...
class AdobeDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for video in video_list:
            ...
            while index + (interval + 1) * 3 < len(frames):
                videoInputs_index = [index, index + (1 + interval)*1, index + (1 + interval)*2, index + (1 + interval)*3]
                videoInputs = [frames[i] for i in videoInputs_index]
                video_all_gt = [frames[i] for i in range(index, index + (1 + interval) * 3 + 1)]
                print(videoInputs, video_all_gt)
